...
def federated_train(client_number,model,client_data,test_data,Var):
    ...
    for i in range(client_number):
        ...
        for epoch in range(args.local_epochs):#客户端训练次数 3次
            ...
            for epoch_ind, (data, target) in enumerate(train_loader):
                ...
                with torch.no_grad():
                    proximal_term = 0.0
                    for w, w_t in zip(client_model[i].parameters(), global_change_model.parameters()):
                        proximal_term += (w - w_t.send(client_list[i])).norm(2)
                loss = F.cross_entropy(pred, target) + (args.mu / 2) * proximal_term
                loss.backward()
                client_optim[i].step()
