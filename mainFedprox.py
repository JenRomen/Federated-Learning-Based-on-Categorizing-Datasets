...
from trainFedprox import federated_train, fedavg_updata_weight, test
...
if __name__ == "__main__":
    ...
    for i in range(args.epochs):
        print(f"----------epoch={i + 1}----------")
        model = federated_train(client_number, model, client_data, test_dataset, Var)
        save_data(Var)
        if i % 10 == 0:
            print("每10回合的模型保存")
    ...
    print("----------5.模型保存----------")
    modelUtils.save_model("fedProx.pt", model)
    dataUtils.save_var("var_fedprox", Var)
